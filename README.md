...
This endpoint is designed to work with a RunPod network volume mounted at `/runpod-volume`. Note that network volumes are optional if you are using Cloudflare R2 for model storage. The following model paths are expected:
...
## Cloudflare R2 Setup (DEPRECATED)

This project previously supported Cloudflare R2 for model storage. This has been removed in favor of direct Hugging Face downloads.
Please ensure `HF_TOKEN` is set in your environment variables to access the required models.

### RunPod Environment Variables
| Variable | Description | Example |
|----------|-------------|---------|
| HF_TOKEN | Hugging Face Token (for authenticating downloads) | hf_... |
| RUNPOD_SERVERLESS | Set to `true` to enable serverless mode. | `false` |
| COMFY_PORT | The port ComfyUI will run on. | `8188` |
